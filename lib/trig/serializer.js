...
class SerializerState {
    ...
    for (let key in obj) {
      ...
      if (revContainer) {
        ...
        for (let i = 0; i < vs.length; i++) {
          ...
          if (term == "a") {
            let t = typeof v === 'string' ? v : v['@type'] // assuming annotation form
            this.write(this.toValidTerm(this.termFor(t)))
  ...
  refRepr (ref, useVocab = false) {
    ...
    // check if ref is *most likely* a pname (SHOULD check for prefix definition!)
    if (ref.indexOf(':') > -1) {
      let [pname, ...rest] = ref.split(/:/)
      if (rest.length == 1 && rest[0].match(/^\w*$/) && pname.match(/^\w*$/)) {
        return ref
      }
    }
