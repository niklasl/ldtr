const {CONTEXT, GRAPH, ID, TYPE, VALUE, LANGUAGE, LIST, VOCAB, BASE} =
  require('../jsonld/keywords')

module.exports = {serialize}

function serialize (data, out = process.stdout, {
  context = null,
  baseIri = null,
  settings = {
    indentChars: '  ',
    upcaseKeywords: false,
    useGraphKeyword: false,
...
class SerializerState {
  ...
  objectToTurtle (obj, depth = 0, viaKey = null) {
    ...
    let indent = this.getIndent(depth)
          ...
          let v = vs[i]
            if (inList) {
              ...
              if (this.settings.predicateRepeatNewLine) {
                this.writeln(' ,')
                this.write(this.getIndent(depth + 1))
              } else {
                this.write(' , ')
              }
            }
    ...
    if (depth == 0) {
      ...
      if (!isList) {
        ...
        if (!this.hasKeys(obj) && markEmptyBnode) {
          this.writeln("a $emptyMarker")
          this.write(indent)
        }
        this.write("]")
      }
  ...
  toLiteral (obj, viaKey=null) {
    ...
    if (typeof obj === 'object') {
      ...
      if (coerceTo == "@vocab") {
        this.write(typeof value === 'string' ?
          this.refRepr(value, true): value)
        return
      } else if (coerceTo == "@id") {
        this.write(this.refRepr(value))
        return
      } else if (coerceTo) {
        datatype = coerceTo
      } else {
    ...
    if (typeof value === 'string') {
      ...
      this.write('"')
      if (datatype)
        this.write("^^" + this.termFor(datatype))
      else if (lang)
        this.write("@" + lang)
