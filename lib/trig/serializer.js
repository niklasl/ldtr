...
import { ANNOTATION, ANNOTATED_TYPE_KEY, QUOTED, TRIPLE } from '../jsonld/star.js'
...
class SerializerState {
  ...
  objectToTurtle (obj, depth = 0, viaKey = null) {
    ...
    if (!inGraph && viaKey != null && typeof obj === 'object' && this.keys.triple in obj) {
      let reifiedTriple = this.reprTriple(obj[this.keys.triple])
      this.write(reifiedTriple)
      return []
    }

    if (typeof obj === 'object' && obj[this.keys.type] == this.keys.triple) {
      let triple = this.reprTriple(obj[this.keys.value])
    ...
    for (let key in obj) {
      ...
      if (revContainer) {
        ...
        for (let i = 0; i < vs.length; i++) {
          ...
          this.outputAnnotation(v, depth, inGraph)
        }
      }
    }
  ...
  outputAnnotation (v, depth, inGraph = false) {
    let annotation = v[this.keys.annotation]
    const annotDepth = depth + (inGraph ? 2 : 1)
  ...
  reprTriple(ref, name=null) {
    ...
    if (p == this.keys.type) {
      ...
      if (typeof obj === 'object' && obj != null && this.keys.list in obj) {
