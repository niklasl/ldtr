...
import { ANNOTATION, ANNOTATED_TYPE_KEY, REIFIES, TRIPLE, QUOTED } from '../jsonld/star.js'
...
class SerializerState {
  ...
  objectToTurtle (obj, depth = 0, viaKey = null) {
    ...
    const inGraph = viaKey === GRAPH ? 1 : 0

    if (!inGraph && viaKey != null && typeof obj === 'object' && this.keys.triple in obj) {
      let tripleTerm = this.reprTriple(obj[this.keys.triple], null, true)
      this.write(tripleTerm)
      return []
    }
    ...
    const tripleObj = (typeof obj === 'object' && this.keys.triple in obj) ? obj[this.keys.triple] : null

    if (((s != null || !viaKey || inGraph)) && this.hasKeys(obj, 2) || tripleObj) {
      const reifying = !(this.keys.reifies in obj)
      ...
      if (tripleObj) {
        let tripleSubject = this.reprTriple(tripleObj, s)
        this.write(tripleSubject)
      } else if (reifying) {
    ...
    if (typeof obj === 'object' && this.keys.reifies in obj) {
      if (s == null) {
        // FIXME [66e292c5]: fails on reified objects! Nest instead...
        s = this.nextBnodeId()
        obj[this.keys.id] = s
      }
      for (const reiObj of asArray(obj[this.keys.reifies])) {
        const reiId = reiObj[this.keys.id]
        for (const reiKey in reiObj) {
          if (reiKey === this.keys.id) continue
          for (const reiValue of asArray(reiObj[reiKey])) {
            const reiTriple = { [this.keys.id]: reiId, [reiKey]: reiValue }
            const reified = { [this.keys.id]: s, [this.keys.triple]: reiTriple }
            topObjects.push(reified)
          }
        }
      }
      const reifier = Object.assign({}, obj)
      delete reifier[this.keys.reifies]
      if (Object.keys(reifier).length > 1) topObjects.push(reifier)

      obj = {}
    }

    for (let key in obj) {
      if (key === this.keys.graph) {
        continue
      }

      if (key === this.keys.triple) {
        continue
      }
      ...
      if (revContainer) {
        ...
        for (let i = 0; i < vs.length; i++) {
          ...
          const isQuoted = typeof v === 'object' && this.keys.quoted in v

          if (isQuoted) {
            this.write("<< ")
            const annot = v[this.keys.annotation]
            v = v[this.keys.quoted]
            if (annot != null) {
              if (typeof v !== 'object') {
                v = {[this.keys.value]: v}
              }
              v[this.keys.annotation] = annot
            }
          }

          // FIXME: [66e292c5]
          if (typeof v === 'object' && this.keys.reifies in v && v[this.keys.id] == null) {
            v[this.keys.id] = this.nextBnodeId()
          }
          ...
          this.outputAnnotation(v, nestedDepth)
        }
      }
    }
  ...
  outputAnnotation (v, annotDepth) {
    ...
    const single = annotation.length === 1

    let first = true
    for (const annot of annotation) {
      if (!first && !single) {
        this.writeln()
        this.write(this.getIndent(annotDepth + 1))
      }

      const id = '@id' in annot ? this.refRepr(annot['@id']) : null
      if (id !== null) {
        if (first) this.write(' ')
        this.write(`~ ${id}`)
      }

      if (id == null || Object.keys(annot).length > 1) {
        this.write(' {|')
        const outAnnot = Object.assign({}, annot)
        delete outAnnot[this.keys.id]
        this.objectToTurtle(outAnnot, annotDepth + 3, this.keys.annotation)
        this.write('|}')
      }
  ...
  termFor (key) {
    if (key.startsWith("@")) {
      ...
      if (typeof kdef === 'object') {
        ...
        term = key in this.prefixes ? key : kdef
      }
      if (term == null)
        return null
  ...
  makeTopObject (s, revKey, it) {
    ...
    if (node[this.keys.id] == null) {
      node[this.keys.id] = this.nextBnodeId()
    }
  ...
  reprTriple(ref, name=null, isTerm=false) {
    ...
    if (p == this.keys.type) {
      ...
      if (isTerm && Array.isArray(obj)) {
        throw 'Quoted triples must have one single object'
      }
      if (isTerm && this.isLangContainer(p) && typeof obj === 'object') {
        throw 'Language containers not yet supported in quoted triples'
      }
      if (isTerm && typeof obj === 'object' && obj != null && this.keys.list in obj) {
        throw 'Quoted triples cannot contain Lists'
      }

      if (!(typeof obj === 'object') || obj != null && obj[this.keys.value]) {
        let l = []
        this.toLiteral(obj, p, s => l.push(s))
        o = l.join('')
      } else {
        o = this.refRepr(obj[ID])
      }
    }
    ...
    const nameRepr = name ? `~ ${this.refRepr(name)} ` : ''
    return `<< ${s} ${p} ${o} ${nameRepr}>>`
  }
  ...
  nextBnodeId () {
    return  `_:bnode-${this.bnodeCounter++}`
  }
