...
import { ANNOTATION, ANNOTATED_TYPE_KEY, QUOTED } from '../jsonld/star.js'
...
class SerializerState {
  ...
  objectToTurtle (obj, depth = 0, viaKey = null) {
    ...
    if ((s != null || !viaKey || inGraph) && this.hasKeys(obj, 2)) {
      if (depth === 0) {
        this.writeln()
      }
  ...
  outputAnnotation (v, depth, inGraph = false) {
    ...
    if (Array.isArray(annotation)) {
      ...
      for (const annot of annotation) {
        if (!single) {
          this.writeln()
          this.write(this.getIndent(annotDepth + 2))
        }
        ...
        if (id !== null) {
          ...
          this.write('[')
          this.objectToTurtle(annot, annotDepth + 3, this.keys.annotation)
          this.write(']')
        }
      ...
      this.write(' {|')
      if ('@id' in annotation) {
        const id = this.refRepr(annotation['@id'])
        this.write(` @ ${id}`)
      }
      this.objectToTurtle(annotation, annotDepth, this.keys.annotation)
      this.write('|}')
    }
  }
