...
async function rdfxmlToJsonLd (input) {
  ...
  if (doc == null) {
    let domParser = new vars.DOMParser()
...
async function xmlToJsonLd (input) {
  // Just assuming RDF/XML. The data *could* be using GRDDL or even XML+RDFa,
  // which should ideally be inspected for.
  return rdfxmlToJsonLd(input)
}
