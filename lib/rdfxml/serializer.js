...
export class RDFXMLSerializer {
  ...
  isRef(value) {
    return ID in value && (Object.keys(value).length === (ANNOTATION in value ? 2 : 1))
  }
  ...
  handleQuotedTriple (triplenode, annot = null) {
    ...
    for (let k in triplenode) {
      if (k != ID) {
        ...
        let o = triplenode[k]
        if (k === TYPE) o = {[ID]: this.resolve(o)}
        this.handleContents({["rdf:object"]: o})
        break
      }
    }
    if (annot != null) {
...
export class XMLWriter {
  ...
  addElement (tag, attrs = null, literal = null) {
    ...
    if (literal != null) {
      this.write(">")
      this.write(xmlescape(literal.toString()))
      this.write("</" + tag + ">\n")
    } else {
      this.write("/>\n")
    }
...
export function makeQid (ctx, triplenode) {
  ...
  if (typeof o === 'string') {
    ...
    if (Object.hasOwnProperty.call(o, ID)) {
      orepr = o[ID]
    } else if (Object.hasOwnProperty.call(o, VALUE)) {
      orepr = o[VALUE] + " "
      let lang = o[LANGUAGE]
      let dt = o[TYPE]
      if (lang != null) {
  ...
  let triplerepr = orepr != "" ? s + " " + p + " " + orepr : null
  if ((triplerepr == null || ctx.tripleIdForm == "_")) {
    return "triple-" + ctx.bnodes.makeBnodeId(triplerepr).substring(2)
  } else if (ctx.tripleIdForm != null) {
    let hash = vars.createHash(ctx.tripleIdForm)
