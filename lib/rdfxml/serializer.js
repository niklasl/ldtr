...
import { BNodes } from '../jsonld/util.js'
...
export class RDFXMLSerializer {
  ...
  constructor (builder, data, useArcIds = true, tripleIdForm = "sha1") {
    this.builder = builder
    this.bnodes = new BNodes()
    this._deferreds = []
...
export function makeQid (ctx, triplenode) {
  ...
  if (ctx.tripleIdForm == "_") {
    return ctx.bnodes.makeBnodeId(triplerepr)
