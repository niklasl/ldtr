...
if (typeof require === 'function') {
  var RDFaWalker = require("./walker");
}

var RDFaJSONLDParser = (function () {
  if (typeof exports === 'undefined') exports = {};

  var ID = '@id';

  exports.parse = function (doc, base, options) {
    if (doc === null) {
      doc = window.document;
    }
  ...
  builder = {
    ...
    visit: function (desc, state) {
      ...
      if (desc.vocab) {
        var baseObj = getOrCreateNode(result, desc.context.base);
        addPropToObj(state, baseObj, state.usesVocabExpr, {
          '@id': desc.vocab
        });
      }

      var activeSubject = desc.subject || desc.parentSubject;
      var currentNode = getOrCreateNode(result, activeSubject);
      var localNode = getOrCreateNode(result, desc.subject || desc.resource);
